...
{/* IMAGE: hero — a calm host at a velvet rope directing arriving guests toward several open doors, one overcrowded door glowing red in the background as a warning */}
...
A **Layer 7** load balancer reads further — the actual web address being requested, the specific hostname, the headers attached to the message — and routes based on what it finds. That's how AWS's Application Load Balancer can send `api.example.com` one way and `web.example.com` another, or split `/api/*` traffic from `/static/*` traffic, even though both arrived at the exact same front door.
...
{/* IMAGE: diagram — split comparison panel: fifty servers with no load balancer, all traffic funneling into one overloaded server, versus the same fifty servers with a load balancer evenly spreading the same traffic across all of them */}

## The scale this runs at

None of this is small. **nginx**, a free, open-source Layer 7 load balancer, is what Facebook uses for its own load balancing, and it can hold millions of concurrent connections at once. **AWS's Elastic Load Balancer** manages this automatically for Amazon's own EC2 servers, with the Application Load Balancer version running roughly $16 a month at typical usage. And **HAProxy** — also free, also open-source, used by GitHub, Shopify, and Twitter — is the load balancer standing behind that fifty-server Black Friday example from the top of this post: without it, one server eats fifty times its normal traffic and crashes; with it, the same fifty servers absorb ten times the normal load without anyone noticing.
